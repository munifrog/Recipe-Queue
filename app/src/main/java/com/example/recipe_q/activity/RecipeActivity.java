...
public class RecipeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null) {
            ...
            Picasso.get().load(recipe.getImage()).placeholder(R.drawable.ic_launcher_background).into(recipeImage);

            ActionBar actionBar = getSupportActionBar();
            if (actionBar != null) {
                actionBar.setTitle(recipe.getRecipeTitle());
                actionBar.setDisplayHomeAsUpEnabled(true);
            }

            AdapterLinearIngredients adapterIngredients = new AdapterLinearIngredients(recipe.getIngredients());
            RecyclerView rvIngredients = findViewById(R.id.rv_ingredients);
            rvIngredients.setLayoutManager(new LinearLayoutManager(this));
            rvIngredients.setAdapter(adapterIngredients);

            AdapterLinearDirectionGroups adapterDirections = new AdapterLinearDirectionGroups(recipe.getDirections());
            ...
            tvServings.setText(String.format(Locale.getDefault(), "%1$.2f", recipe.getServings()));
            TextView tvReadyIn = findViewById(R.id.tv_ready_in_minutes);
            tvReadyIn.setText(String.format(Locale.getDefault(), "%1$d", recipe.getReadyInMinutes()));
